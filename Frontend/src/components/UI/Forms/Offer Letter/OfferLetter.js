...
const OfferLetter = () => {
  ...
  useEffect(() => {
    if (id) {
      axios
        .get(`https://generette.onrender.com/edit/offer_letter/${id}`)
        .then((response) => {
  ...
  const handleSubmit = async (event) => {
    ...
    const obj = {
      ...
      jobDetails: {
        ...
        responsibilitiesAndExpectations:
          refs.jobDetails.responsibilitiesAndExpectations,
    ...
    await fetch("https://generette.onrender.com/offer_letter", {
      method: "POST",
      headers: {
  ...
  switch (index) {
    ...
    case 3:
      content = (
        <JobDetails
          ...
          nextStep={nextStep}
          prevStep={prevStep}
          onChangeSub={fieldSubChangeHandler}
          increaseSub={fieldSubAddHandler}
          decreaseSub={fieldSubDecreaseHandler}
        />
      );
      break;
    case 4:
    ...
    case 6:
      ...
      break;
    default:
      content = <FinalPage title="Thank You!" values={refs} />;
  }
  return <>{content}</>;
};
